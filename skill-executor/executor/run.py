...
def _scrub_logs(logs: str, secret_values: dict[str, str] | None = None) -> str:
    literals = [
        value
        for value in (secret_values or {}).values()
    ...
    for line in logs.splitlines():
        ...
        for value in literals:
            replaced = replaced.replace(value, "[scrubbed secret value]")
        if SECRET_RE.search(replaced):
            scrubbed_lines.append("[scrubbed potentially sensitive log line]")
        else:
            scrubbed_lines.append(replaced)
    return "\n".join(scrubbed_lines)
...
def _fetch_secrets() -> dict[str, str]:
    fetch_url = os.environ.get("KOINOFLOW_SECRETS_FETCH_URL", "").strip()
    fetch_token = os.environ.get("KOINOFLOW_SECRETS_FETCH_TOKEN", "").strip()
    if not fetch_url or not fetch_token:
        return {}
    ...
    if not isinstance(values, dict):
        raise RuntimeError("Secrets fetch response was invalid")
    return {
        str(name): str(value)
...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"koinoflow-{run_id}-") as tmp:
            ...
            if (
                not str(entrypoint).startswith(str(workdir.resolve()))
                or not entrypoint.is_file()
            ):
                raise RuntimeError(
                    "Execution entrypoint is missing or outside the skill package"
                )

            timeout_seconds = int(
                manifest.get("limits", {}).get("timeout_seconds") or 30
            )
            secret_values = _fetch_secrets()
            child_env = {
            ...
            logs = _scrub_logs(process.stderr, secret_values=secret_values)
            ...
            max_inline = int(
                manifest.get("limits", {}).get("max_output_bytes_inline") or 32768
            )
            inline_output = (
                output if len(output_text.encode("utf-8")) <= max_inline else None
            )
            _callback(
                callback_url,
                callback_token,
