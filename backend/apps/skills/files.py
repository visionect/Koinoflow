...
def resolve_files(skill_id: uuid.UUID, version_number: int) -> dict[str, VersionFile]:
    ...
    rows = VersionFile.objects.raw(
        ...
        JOIN skill_version sv ON sv.id = vf.version_id
        WHERE sv.skill_id = %s
          AND sv.version_number <= %s
        ORDER BY vf.path, sv.version_number DESC
        """,
        [str(skill_id), version_number],
    )
    return {r.path: r for r in rows if not r.is_deleted}


def resolve_file_list(skill_id: uuid.UUID, version_number: int) -> list[dict]:
    """Lightweight listing: returns path + file_type + size only (no content)."""
    files = resolve_files(skill_id, version_number)
    return [
        {
            "id": str(f.id),
            "path": f.path,
...
def compute_file_delta(
    ...
    """
    old = resolve_files(skill_id, previous_version_number) if previous_version_number else {}
