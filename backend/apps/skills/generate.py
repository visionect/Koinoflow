...
def _get_generation_model():
    ...
    model_name = getattr(settings, "SKILL_GENERATION_MODEL", "gemini-3-flash-preview")
    return _build_provider(model_name, settings)


def generate_skill_from_text(source_text: str) -> tuple[str, str]:
    """
    Call the LLM to transform source_text into a SKILL.md, then split it into
    (frontmatter_yaml, content_md) matching SkillVersion fields.
