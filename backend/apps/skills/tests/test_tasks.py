...
from apps.skills.tasks import send_staleness_alert, staleness_check
from apps.skills.tests.factories import SkillFactory


def _create_usage_event(skill, days_ago):
...
class TestStalenessCheck:
    def _make_stale_skill(self, **kwargs):
        ws = WorkspaceFactory()
        team = TeamFactory(workspace=ws)
        dept = DepartmentFactory(team=team)
        owner = UserFactory()
        rule = StalenessAlertRuleFactory(workspace=ws, period_days=30)
        CoreSettingsFactory(workspace=ws, staleness_alert=rule, skill_audit=None)
        defaults = {"department": dept, "status": "published", "owner": owner}
        defaults.update(kwargs)
        skill = SkillFactory(**defaults)
        # No UsageEvent created → never used → stale
        return skill

    @patch("tasks.task_backend")
    def test_staleness_check_finds_never_used_skill(self, mock_backend):
        skill = self._make_stale_skill()

        staleness_check()

        mock_backend.enqueue.assert_called_once_with("send_staleness_alert", skill_id=str(skill.id))

    @patch("tasks.task_backend")
    def test_staleness_check_finds_skill_unused_beyond_period(self, mock_backend):
        skill = self._make_stale_skill()
        _create_usage_event(skill, days_ago=35)  # used 35 days ago, period is 30
    ...
    @patch("tasks.task_backend")
    def test_staleness_check_ignores_recently_used_skill(self, mock_backend):
        skill = self._make_stale_skill()
        _create_usage_event(skill, days_ago=5)  # used 5 days ago, period is 30
    ...
    def test_staleness_check_ignores_draft_processes(self, mock_backend):
        self._make_stale_skill(status="draft")
    ...
    def test_staleness_check_requires_staleness_rule(self, mock_backend):
        ...
        dept = DepartmentFactory(team=team)
        owner = UserFactory()
        CoreSettingsFactory(workspace=ws, staleness_alert=None, skill_audit=None)
        SkillFactory(department=dept, status="published", owner=owner)
...
class TestSendStalenessAlert:
    def _make_skill_with_rule(self, notify_admins=True, notify_skill_owner=True):
        ws = WorkspaceFactory()
        team = TeamFactory(workspace=ws)
        dept = DepartmentFactory(team=team)
        owner = UserFactory()
        ...
        rule = StalenessAlertRuleFactory(
            ...
            notify_skill_owner=notify_skill_owner,
        )
        CoreSettingsFactory(workspace=ws, staleness_alert=rule, skill_audit=None)
        skill = SkillFactory(department=dept, status="published", owner=owner)
        return skill, admin_user, owner
    ...
    def test_send_staleness_alert_emails_admin(self, mock_get_backend):
        ...
        skill, admin_user, owner = self._make_skill_with_rule(
            notify_admins=True, notify_skill_owner=False
        )

        send_staleness_alert(str(skill.id))
    ...
    def test_send_staleness_alert_emails_owner(self, mock_get_backend):
        ...
        mock_get_backend.return_value = mock_backend
        skill, admin_user, owner = self._make_skill_with_rule(
            notify_admins=False, notify_skill_owner=True
        )
    ...
    def test_send_staleness_alert_no_recipients_skips(self, mock_get_backend):
        ...
        CoreSettingsFactory(workspace=ws, staleness_alert=rule, skill_audit=None)
        skill = SkillFactory(department=dept, status="published", owner=None)
    ...
    @patch("apps.common.email_service.get_email_backend")
    def test_send_staleness_alert_subject_contains_skill_title(self, mock_get_backend):
        mock_backend = MagicMock()
        mock_get_backend.return_value = mock_backend
        skill, _, _ = self._make_skill_with_rule()

        send_staleness_alert(str(skill.id))

        subject = mock_backend.send.call_args.kwargs["subject"]
        assert skill.title in subject

    @patch("apps.common.email_service.get_email_backend")
    def test_send_staleness_alert_days_stale_from_usage_event(self, mock_get_backend):
        mock_backend = MagicMock()
        mock_get_backend.return_value = mock_backend
        skill, _, _ = self._make_skill_with_rule(notify_admins=True, notify_skill_owner=False)
        _create_usage_event(skill, days_ago=45)
    ...
    @patch("apps.common.email_service.get_email_backend")
    def test_send_staleness_alert_nonexistent_skill(self, mock_get_backend):
        mock_backend = MagicMock()
        mock_get_backend.return_value = mock_backend
