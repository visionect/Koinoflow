...
def get_embedding_config() -> EmbeddingConfig:
    dimensions = int(getattr(settings, "PROCESS_DISCOVERY_EMBEDDING_DIMENSIONS", 768))
    if dimensions != SUPPORTED_EMBEDDING_DIMENSIONS:
        raise ValueError(
            "PROCESS_DISCOVERY_EMBEDDING_DIMENSIONS must be 768 until the vector "
            "column dimension is migrated."
        )
    return EmbeddingConfig(
        model=getattr(settings, "PROCESS_DISCOVERY_EMBEDDING_MODEL", "gemini-embedding-2"),
...
class VertexEmbeddingClient:
    ...
    def embed_document(self, text: str) -> list[float]:
        prompt = f"Represent this Koinoflow process for retrieval by a user's task.\n\n{text}"
...
def index_process_version(
    version_id: str, *, force: bool = False
) -> ProcessDiscoveryEmbedding | None:
    version = (
        ProcessVersion.objects.select_related("process__department__team")
    ...
    process = version.process
    if process.status != StatusChoices.PUBLISHED or process.current_version_id != version.id:
        ProcessDiscoveryEmbedding.objects.filter(version=version).delete()
        return None
    ...
    embedding, _ = ProcessDiscoveryEmbedding.objects.update_or_create(
        version=version,
        defaults={
            "embedding": vector,
...
def queue_process_discovery_embedding(version_id: str, *, force: bool = False) -> None:
    try:
        from apps.processes.celery_tasks import index_process_discovery_embedding_task

        index_process_discovery_embedding_task.delay(str(version_id), force=force)
    except Exception:
        logger.warning("Failed to queue process discovery embedding", exc_info=True)
