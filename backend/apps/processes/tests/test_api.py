...
class TestCreateProcess:
    def test_create_process_without_version(self, auth_client, admin_membership):
        ...
        process = Process.objects.get(slug="deploy-process")
        assert process.versions.count() == 0

    def test_create_process_duplicate_slug(self, auth_client, admin_membership):
        ws = admin_membership.workspace
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
        ProcessFactory(department=dept, slug="existing")
...
class TestGetProcess:
    def test_get_process_published(self, auth_client, admin_membership):
        ws = admin_membership.workspace
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
        process = ProcessFactory(department=dept, slug="deploy", status=StatusChoices.PUBLISHED)
        version = ProcessVersionFactory(process=process, version_number=1, content_md="# Deploy")
    ...
    def test_get_process_draft_via_session(self, auth_client, admin_membership):
        ...
        resp = auth_client.get("/api/v1/processes/draft-proc")
        assert resp.status_code == 200

    def test_get_process_draft_via_api_key(self, admin_membership):
        ws = admin_membership.workspace
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
        ProcessFactory(department=dept, slug="draft-proc", status=StatusChoices.DRAFT)
...
class TestUpdateProcess:
    def test_update_process_title(self, auth_client, admin_membership):
        ...
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
        ProcessFactory(department=dept, slug="deploy", title="Old Title")

        resp = auth_client.patch(
            "/api/v1/processes/deploy",
...
class TestUpdateProcessScope:
    def test_admin_can_change_visibility_to_workspace(self, auth_client, admin_membership):
        ...
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
        ProcessFactory(department=dept, slug="deploy", visibility=VisibilityChoices.DEPARTMENT)
    ...
    def test_team_manager_cannot_set_workspace_visibility(
        ...
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
        ProcessFactory(department=dept, slug="deploy", visibility=VisibilityChoices.TEAM)
    ...
    def test_admin_can_change_workspace_to_team(self, auth_client, admin_membership):
        ...
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
        ProcessFactory(department=dept, slug="deploy", visibility=VisibilityChoices.WORKSPACE)
    ...
    def test_update_shared_with_ids(self, auth_client, admin_membership):
        ...
        ProcessFactory(department=dept1, slug="deploy", visibility=VisibilityChoices.DEPARTMENT)
    ...
    def test_shared_with_invalid_dept_returns_400(self, auth_client, admin_membership):
        ...
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
        ProcessFactory(department=dept, slug="deploy")
...
class TestUnshareFromMyTeam:
    def test_team_manager_can_unshare_from_own_team(
        ...
        process = ProcessFactory(department=owner_dept, slug="shared-proc")

        mgr_team = team_manager_membership.team
        mgr_dept = DepartmentFactory(team=mgr_team)
        process.shared_with.add(mgr_dept)

        client = Client()
        client.force_login(team_manager_membership.user)
        resp = client.delete("/api/v1/processes/shared-proc/shared-with/my-team")
    ...
    def test_unshare_does_not_affect_other_teams(self, admin_membership, team_manager_membership):
        ...
        process.shared_with.add(mgr_dept, other_dept)
    ...
    def test_returns_400_when_not_shared(self, admin_membership, team_manager_membership):
        ...
        ProcessFactory(department=dept, slug="not-shared")

        client = Client()
        client.force_login(team_manager_membership.user)
        resp = client.delete("/api/v1/processes/not-shared/shared-with/my-team")
    ...
    def test_admin_cannot_use_endpoint(self, auth_client, admin_membership):
        ...
        team = TeamFactory(workspace=ws)
        dept = DepartmentFactory(team=team)
        process = ProcessFactory(department=dept, slug="proc")
        other_dept = DepartmentFactory(team=TeamFactory(workspace=ws))
        process.shared_with.add(other_dept)

        resp = auth_client.delete("/api/v1/processes/proc/shared-with/my-team")
        assert resp.status_code == 403

    def test_get_process_reflects_is_shared_with_requester_team(
        self, admin_membership, team_manager_membership
    ):
        ws = admin_membership.workspace
        team = TeamFactory(workspace=ws)
        dept = DepartmentFactory(team=team)
        process = ProcessFactory(department=dept, slug="shared-proc")
        ...
        client = Client()
        client.force_login(team_manager_membership.user)
        resp = client.get("/api/v1/processes/shared-proc")
...
class TestDeleteProcess:
    def test_delete_process(self, auth_client, admin_membership):
        ws = admin_membership.workspace
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
        ProcessFactory(department=dept, slug="deploy")

        resp = auth_client.delete("/api/v1/processes/deploy")
        assert resp.status_code == 200
        assert resp.json() == {"ok": True}
        assert not Process.objects.filter(slug="deploy").exists()
...
class TestCreateVersion:
    def test_create_version_increments_number(self, auth_client, admin_membership):
        ...
        resp = auth_client.post(
            "/api/v1/processes/deploy/versions",
            data={
...
class TestListVersions:
    def test_list_versions(self, auth_client, admin_membership):
        ...
        resp = auth_client.get("/api/v1/processes/deploy/versions")
        assert resp.status_code == 200
        data = resp.json()
        assert data["count"] == 2
...
class TestGetVersion:
    def test_get_specific_version(self, auth_client, admin_membership):
        ...
        dept = DepartmentFactory(team=team, slug="frontend")
        process = ProcessFactory(department=dept, slug="deploy")
        ProcessVersionFactory(process=process, version_number=1, content_md="# V1")
        ProcessVersionFactory(process=process, version_number=2, content_md="# V2")

        resp = auth_client.get("/api/v1/processes/deploy/versions/1")
        assert resp.status_code == 200
...
class TestVersionDiff:
    def test_diff_returns_hunks(self, auth_client, admin_membership):
        ...
        process = ProcessFactory(department=dept, slug="deploy")
        ProcessVersionFactory(
            process=process, version_number=1, content_md="# Deploy\n\nStep 1: Build"
        )
        ProcessVersionFactory(
            process=process, version_number=2, content_md="# Deploy\n\nStep 1: Build\nStep 2: Ship"
        )

        resp = auth_client.get("/api/v1/processes/deploy/versions/2/diff")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_diff_v1_returns_400(self, auth_client, admin_membership):
        ...
        dept = DepartmentFactory(team=team, slug="frontend")
        process = ProcessFactory(department=dept, slug="deploy")
        ProcessVersionFactory(process=process, version_number=1, content_md="# Deploy")

        resp = auth_client.get("/api/v1/processes/deploy/versions/1/diff")
        assert resp.status_code == 400
    ...
    def test_diff_nonexistent_version_returns_404(self, auth_client, admin_membership):
        ...
        resp = auth_client.get("/api/v1/processes/deploy/versions/99/diff")
        assert resp.status_code == 404
...
class TestPublishProcess:
    def test_publish_updates_current_version(self, auth_client, admin_membership):
        ...
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
        process = ProcessFactory(department=dept, slug="deploy", status=StatusChoices.DRAFT)
        ProcessVersionFactory(process=process, version_number=1)
        v2 = ProcessVersionFactory(process=process, version_number=2)

        resp = auth_client.post("/api/v1/processes/deploy/publish")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert process.current_version_id == v2.id


@pytest.mark.django_db
class TestListProcesses:
    def test_list_processes_basic(self, auth_client, admin_membership):
        ws = admin_membership.workspace
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
        ProcessFactory(department=dept, slug="p1", title="First")
        ProcessFactory(department=dept, slug="p2", title="Second")
    ...
    def test_list_processes_search(self, auth_client, admin_membership):
        ws = admin_membership.workspace
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
        ProcessFactory(department=dept, slug="deploy", title="Deploy Process")
        ProcessFactory(department=dept, slug="onboard", title="Onboarding Guide")

        resp = auth_client.get("/api/v1/processes?search=deploy")
        assert resp.status_code == 200
        data = resp.json()
        assert data["count"] == 1
    ...
    def test_list_processes_api_key_hides_drafts(self, admin_membership):
        ...
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
        ProcessFactory(department=dept, slug="draft", status=StatusChoices.DRAFT)
        ProcessFactory(department=dept, slug="published", status=StatusChoices.PUBLISHED)
...
class TestCreateProcessSlugUniqueness:
    def test_workspace_wide_slug_uniqueness(self, auth_client, admin_membership):
        ...
        dept1 = DepartmentFactory(team=team, slug="frontend")
        dept2 = DepartmentFactory(team=team, slug="backend")
        ProcessFactory(department=dept1, slug="deploy")
...
class TestProcessVisibility:
    def test_create_process_with_default_visibility(self, auth_client, admin_membership):
        ws = admin_membership.workspace
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
    ...
    def test_create_process_with_team_visibility(self, auth_client, admin_membership):
        ws = admin_membership.workspace
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
    ...
    def test_create_process_with_workspace_visibility(self, auth_client, admin_membership):
        ws = admin_membership.workspace
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
    ...
    def test_create_process_with_invalid_visibility(self, auth_client, admin_membership):
        ws = admin_membership.workspace
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
    ...
    def test_visibility_in_list_response(self, auth_client, admin_membership):
        ...
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
        ProcessFactory(department=dept, slug="p1", visibility=VisibilityChoices.TEAM)
    ...
    def test_workspace_wide_survives_team_filter(self, auth_client, admin_membership):
        ...
        ProcessFactory(department=dept_a, slug="ws-skill", visibility=VisibilityChoices.WORKSPACE)
        ProcessFactory(
            department=dept_b, slug="local-skill", visibility=VisibilityChoices.DEPARTMENT
        )

        resp = auth_client.get("/api/v1/processes?department=finance&team=ops")
        assert resp.status_code == 200
        slugs = {item["slug"] for item in resp.json()["items"]}
...
class TestSharedWith:
    def test_create_process_with_shared_with(self, auth_client, admin_membership):
        ws = admin_membership.workspace
        team = TeamFactory(workspace=ws, slug="eng")
        dept = DepartmentFactory(team=team, slug="frontend")
        dept2 = DepartmentFactory(team=team, slug="backend")
...
class TestReviewProcess:
    def test_mark_as_reviewed(self, auth_client, admin_membership):
        ...
        resp = auth_client.post("/api/v1/processes/deploy/review")
        assert resp.status_code == 200
        data = resp.json()
        assert data["last_reviewed_at"] is not None

        process.refresh_from_db()
        assert process.last_reviewed_at is not None
    ...
    def test_review_resets_needs_audit(self, auth_client, admin_membership):
        ...
        auth_client.post("/api/v1/processes/deploy/review")
    ...
    def test_mark_as_reviewed_unassigned_member_forbidden(self, member_membership):
        ...
        dept = DepartmentFactory(team=team, slug="frontend")
        ProcessFactory(department=dept, slug="deploy", status=StatusChoices.PUBLISHED)

        client = Client()
        client.force_login(member_membership.user)
        resp = client.post("/api/v1/processes/deploy/review")
        assert resp.status_code == 403
...
class TestUpdateVersionSummary:
    def test_update_change_summary(self, auth_client, admin_membership):
        ...
        dept = DepartmentFactory(team=team, slug="frontend")
        process = ProcessFactory(department=dept, slug="deploy")
        ProcessVersionFactory(process=process, version_number=1, change_summary="Old summary")

        resp = auth_client.patch(
            "/api/v1/processes/deploy/versions/1",
    ...
    def test_update_nonexistent_version_returns_404(self, auth_client, admin_membership):
        ...
        resp = auth_client.patch(
            "/api/v1/processes/deploy/versions/99",
    ...
    def test_update_summary_persists(self, auth_client, admin_membership):
        ...
        dept = DepartmentFactory(team=team, slug="frontend")
        process = ProcessFactory(department=dept, slug="deploy")
        ProcessVersionFactory(process=process, version_number=1, change_summary="Before")
...
class TestPublishRequireChangeSummary:
    def test_publish_v1_allowed_even_when_summary_required(self, auth_client, admin_membership):
        ...
        process = ProcessFactory(department=dept, slug="deploy", status=StatusChoices.DRAFT)
        ProcessVersionFactory(process=process, version_number=1, change_summary="")
    ...
    def test_publish_blocked_when_summary_required_and_empty(self, auth_client, admin_membership):
        ...
        process = ProcessFactory(department=dept, slug="deploy", status=StatusChoices.DRAFT)
        ProcessVersionFactory(process=process, version_number=1)
        ProcessVersionFactory(process=process, version_number=2, change_summary="")
...
class TestCreateVersionWithFiles:
    def test_create_version_with_files(self, auth_client, admin_membership):
        ...
        ProcessFactory(department=dept, slug="proc-files")

        resp = auth_client.post(
            "/api/v1/processes/proc-files/versions",
            data={
                "content_md": "# Hello",
                "files": [
        ...
        assert VersionFile.objects.filter(version__process__slug="proc-files").count() == 2

    def test_create_version_rejects_invalid_file_type(self, auth_client, admin_membership):
        dept = _make_dept_for_files(admin_membership.workspace)
        ProcessFactory(department=dept, slug="proc-files-invalid-type")

        resp = auth_client.post(
            "/api/v1/processes/proc-files-invalid-type/versions",
            data={
                "content_md": "# Hello",
                "files": [
...
class TestCreateVersionFileDelta:
    def test_file_delta_across_versions(self, auth_client, admin_membership):
        dept = _make_dept_for_files(admin_membership.workspace)
        ProcessFactory(department=dept, slug="proc-delta")

        auth_client.post(
            "/api/v1/processes/proc-delta/versions",
            data={
                "content_md": "# v1",
                "files": [
        ...
        resp_v1 = auth_client.get("/api/v1/processes/proc-delta/versions/1")
...
class TestGetFileContent:
    def test_get_file_content(self, auth_client, admin_membership):
        dept = _make_dept_for_files(admin_membership.workspace)
        process = ProcessFactory(department=dept, slug="proc-fc")
        ...
        resp = auth_client.get("/api/v1/processes/proc-fc/versions/1/files/scripts/run.py")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_get_missing_file_returns_404(self, auth_client, admin_membership):
        dept = _make_dept_for_files(admin_membership.workspace)
        process = ProcessFactory(department=dept, slug="proc-fc2")
        ProcessVersionFactory(process=process, version_number=1)

        resp = auth_client.get("/api/v1/processes/proc-fc2/versions/1/files/missing.py")
        assert resp.status_code == 404
...
class TestFileDiff:
    def test_file_diff_shows_changes(self, auth_client, admin_membership):
        dept = _make_dept_for_files(admin_membership.workspace)
        process = ProcessFactory(department=dept, slug="proc-fd")
        v1 = ProcessVersionFactory(process=process, version_number=1, content_md="# v1")
        ...
        v2 = ProcessVersionFactory(process=process, version_number=2, content_md="# v2")
        ...
        resp = auth_client.get("/api/v1/processes/proc-fd/versions/2/file-diff")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_file_diff_v1_returns_400(self, auth_client, admin_membership):
        dept = _make_dept_for_files(admin_membership.workspace)
        process = ProcessFactory(department=dept, slug="proc-fd2")
        ProcessVersionFactory(process=process, version_number=1)

        resp = auth_client.get("/api/v1/processes/proc-fd2/versions/1/file-diff")
        assert resp.status_code == 400
...
class TestVersionDiffIncludesFileDiff:
    def test_diff_includes_file_diff(self, auth_client, admin_membership):
        dept = _make_dept_for_files(admin_membership.workspace)
        process = ProcessFactory(department=dept, slug="proc-vfd")
        v1 = ProcessVersionFactory(process=process, version_number=1, content_md="# v1")
        VersionFileFactory(version=v1, path="run.py", content="v1", file_type="python")
        v2 = ProcessVersionFactory(process=process, version_number=2, content_md="# v2")
        ...
        resp = auth_client.get("/api/v1/processes/proc-vfd/versions/2/diff")
        assert resp.status_code == 200
        data = resp.json()
...
class TestExportWithFiles:
    def test_export_includes_support_files(self, auth_client, admin_membership):
        dept = _make_dept_for_files(admin_membership.workspace)
        process = ProcessFactory(department=dept, slug="proc-exp", status=StatusChoices.PUBLISHED)
        v1 = ProcessVersionFactory(process=process, version_number=1, content_md="# Export")
        VersionFileFactory(version=v1, path="scripts/run.py", content="print()", file_type="python")
        process.current_version = v1
        process.save()

        resp = auth_client.get("/api/v1/processes/proc-exp/export")
        assert resp.status_code == 200
...
class TestImportWithFiles:
    def test_import_extracts_support_files(self, auth_client, admin_membership):
        ...
        dept = _make_dept_for_files(admin_membership.workspace)
        ProcessFactory(department=dept, slug="proc-imp")
        ...
        resp = auth_client.post(
            "/api/v1/processes/proc-imp/import",
            data={"file": uploaded},
        )
        assert resp.status_code == 201
        data = resp.json()
        ...
        files_resp = auth_client.get(f"/api/v1/processes/proc-imp/versions/{version_number}/files")
    ...
    def test_import_rejects_path_traversal_entries(self, auth_client, admin_membership):
        ...
        dept = _make_dept_for_files(admin_membership.workspace)
        ProcessFactory(department=dept, slug="proc-imp-path")
        ...
        resp = auth_client.post(
            "/api/v1/processes/proc-imp-path/import",
            data={"file": uploaded},
        )
...
class TestKoinoflowMetadata:
    def test_create_version_persists_metadata(self, auth_client, admin_membership):
        dept = _make_dept_for_files(admin_membership.workspace)
        ProcessFactory(department=dept, slug="proc-meta")

        resp = auth_client.post(
            "/api/v1/processes/proc-meta/versions",
            data={
                "content_md": "# Hello",
                "koinoflow_metadata": {
    ...
    def test_create_version_defaults_metadata_when_absent(self, auth_client, admin_membership):
        dept = _make_dept_for_files(admin_membership.workspace)
        ProcessFactory(department=dept, slug="proc-meta-default")

        resp = auth_client.post(
            "/api/v1/processes/proc-meta-default/versions",
    ...
    def test_metadata_change_alone_creates_new_version(self, auth_client, admin_membership):
        dept = _make_dept_for_files(admin_membership.workspace)
        ProcessFactory(department=dept, slug="proc-meta-bump")

        auth_client.post(
            "/api/v1/processes/proc-meta-bump/versions",
    ...
    def test_metadata_never_in_exported_skill_md(self, auth_client, admin_membership):
        ...
        resp = auth_client.get("/api/v1/processes/proc-meta-export/export")
        assert resp.status_code == 200
    ...
    def test_export_includes_metadata_sidecar(self, auth_client, admin_membership):
        ...
        resp = auth_client.get("/api/v1/processes/proc-meta-sidecar/export")
        assert resp.status_code == 200
    ...
    def test_export_empty_metadata_omits_sidecar(self, auth_client, admin_membership):
        ...
        resp = auth_client.get("/api/v1/processes/proc-meta-empty/export")
        assert resp.status_code == 200
    ...
    def test_import_without_sidecar_leaves_metadata_empty(self, auth_client, admin_membership):
        ...
        dept = _make_dept_for_files(admin_membership.workspace)
        ProcessFactory(department=dept, slug="proc-meta-plain-import")
        ...
        resp = auth_client.post(
            "/api/v1/processes/proc-meta-plain-import/import",
            data={"file": uploaded},
        )
        assert resp.status_code == 201
        version_number = resp.json()["version_number"]
        ...
        v_resp = auth_client.get(
            f"/api/v1/processes/proc-meta-plain-import/versions/{version_number}"
        )
    ...
    def test_import_with_sidecar_hydrates_metadata(self, auth_client, admin_membership):
        ...
        dept = _make_dept_for_files(admin_membership.workspace)
        ProcessFactory(department=dept, slug="proc-meta-sidecar-import")
        ...
        uploaded = SimpleUploadedFile("proc.skill", buf.read(), content_type="application/zip")
        resp = auth_client.post(
            "/api/v1/processes/proc-meta-sidecar-import/import",
            data={"file": uploaded},
        )
        assert resp.status_code == 201
        version_number = resp.json()["version_number"]
        ...
        v_resp = auth_client.get(
            f"/api/v1/processes/proc-meta-sidecar-import/versions/{version_number}"
        )
        md = v_resp.json()["koinoflow_metadata"]
    ...
    def test_import_sidecar_not_in_support_files(self, auth_client, admin_membership):
        ...
        dept = _make_dept_for_files(admin_membership.workspace)
        ProcessFactory(department=dept, slug="proc-meta-sidecar-not-file")
        ...
        resp = auth_client.post(
            "/api/v1/processes/proc-meta-sidecar-not-file/import",
            data={"file": uploaded},
        )
        assert resp.status_code == 201
        version_number = resp.json()["version_number"]
        ...
        files_resp = auth_client.get(
            f"/api/v1/processes/proc-meta-sidecar-not-file/versions/{version_number}/files"
        )
        paths = {f["path"] for f in files_resp.json()}
    ...
    def test_rejects_invalid_risk_level(self, auth_client, admin_membership):
        dept = _make_dept_for_files(admin_membership.workspace)
        ProcessFactory(department=dept, slug="proc-meta-invalid")

        resp = auth_client.post(
            "/api/v1/processes/proc-meta-invalid/versions",
            data={
                "content_md": "# x",
...
def _setup_revert_process(ws, slug="revert-proc"):
    team = TeamFactory(workspace=ws)
    dept = DepartmentFactory(team=team)
    return ProcessFactory(department=dept, slug=slug)


@pytest.mark.django_db
class TestRevertVersion:
    def test_creates_new_version_with_target_content(self, auth_client, admin_membership):
        process = _setup_revert_process(admin_membership.workspace)
        ProcessVersionFactory(process=process, version_number=1, content_md="# V1")
        ProcessVersionFactory(process=process, version_number=2, content_md="# V2")
        ProcessVersionFactory(process=process, version_number=3, content_md="# V3")

        resp = auth_client.post(
            f"/api/v1/processes/{process.slug}/versions/1/revert",
            data={},
            content_type="application/json",
        )
        assert resp.status_code == 201
        ...
        assert ProcessVersion.objects.filter(process=process).count() == 4

    def test_custom_change_summary_is_preserved(self, auth_client, admin_membership):
        process = _setup_revert_process(admin_membership.workspace, slug="revert-summary")
        ProcessVersionFactory(process=process, version_number=1, content_md="# V1")
        ProcessVersionFactory(process=process, version_number=2, content_md="# V2")
    ...
    def test_file_tombstone_on_revert(self, auth_client, admin_membership):
        ...
        process = _setup_revert_process(admin_membership.workspace, slug="revert-files")

        # v1: a.py + b.md
        auth_client.post(
            f"/api/v1/processes/{process.slug}/versions",
            data={
                "content_md": "# V1",
                "files": [
        ...
        resolved = resolve_files(process.id, 3)
    ...
    def test_file_restored_on_revert(self, auth_client, admin_membership):
        ...
        process = _setup_revert_process(admin_membership.workspace, slug="revert-restore")
    ...
    def test_identity_rejection(self, auth_client, admin_membership):
        ...
        process = _setup_revert_process(admin_membership.workspace, slug="revert-identical")
        ProcessVersionFactory(process=process, version_number=1, content_md="# V1")
        ProcessVersionFactory(process=process, version_number=2, content_md="# V1")
    ...
    def test_target_not_found_returns_404(self, auth_client, admin_membership):
        process = _setup_revert_process(admin_membership.workspace, slug="revert-404")
        ProcessVersionFactory(process=process, version_number=1, content_md="# V1")

        resp = auth_client.post(
            f"/api/v1/processes/{process.slug}/versions/99/revert",
            data={},
            content_type="application/json",
        )
        assert resp.status_code == 404
    ...
    def test_reverting_to_latest_returns_409(self, auth_client, admin_membership):
        process = _setup_revert_process(admin_membership.workspace, slug="revert-latest")
        ProcessVersionFactory(process=process, version_number=1, content_md="# V1")
        ProcessVersionFactory(process=process, version_number=2, content_md="# V2")

        resp = auth_client.post(
            f"/api/v1/processes/{process.slug}/versions/2/revert",
            data={},
            content_type="application/json",
        )
        assert resp.status_code == 409
    ...
    def test_reverted_from_version_number_in_list(self, auth_client, admin_membership):
        ...
        process = _setup_revert_process(admin_membership.workspace, slug="revert-list")
        ProcessVersionFactory(process=process, version_number=1, content_md="# V1")
        ProcessVersionFactory(process=process, version_number=2, content_md="# V2")
        ...
        resp = auth_client.get(f"/api/v1/processes/{process.slug}/versions")
        assert resp.status_code == 200
...
def _make_oauth_token(user, scope="processes:read processes:write usage:write"):
...
class TestCreateVersionAgentSetting:
    """Verify allow_agent_process_updates gates OAuth-based version creation."""
    ...
    def _post_version(self, client, slug, token=None):
        ...
        return client.post(
            f"/api/v1/processes/{slug}/versions",
            data={
                "content_md": "# Updated",
                "frontmatter_yaml": "",
    ...
    def test_oauth_blocked_when_setting_not_set(self, admin_membership):
        ...
        team = TeamFactory(workspace=ws)
        dept = DepartmentFactory(team=team)
        process = ProcessFactory(department=dept, status=StatusChoices.PUBLISHED)
        ProcessVersionFactory(process=process, version_number=1, content_md="# Original")

        token = _make_oauth_token(admin_membership.user)
        client = Client()
        resp = self._post_version(client, process.slug, token)
        assert resp.status_code == 403
    ...
    def test_oauth_blocked_when_setting_false(self, admin_membership):
        ...
        ProcessVersionFactory(process=process, version_number=1, content_md="# Original")
        CoreSettings.objects.create(
            workspace=ws, team=None, department=None, allow_agent_process_updates=False
        )
    ...
    def test_oauth_allowed_when_setting_true(self, admin_membership):
        ...
        ProcessVersionFactory(process=process, version_number=1, content_md="# Original")
        CoreSettings.objects.create(
            workspace=ws, team=None, department=None, allow_agent_process_updates=True
        )
    ...
    def test_session_auth_not_affected_by_setting(self, auth_client, admin_membership):
        ...
        resp = self._post_version(auth_client, process.slug)
        assert resp.status_code == 201
