...
export function SkillCreateDialog({
  ...
  async function handleCreateSkill() {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Unable to create skill")
    } finally {
  ...
  const pending = createSkill.isPending || isSubmitting

  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{isImport ? "Import skill" : "Create skill"}</DialogTitle>
          <DialogDescription>
            {isImport
        ...
        <div className="space-y-4">
          ...
          {!lockDepartment ? (
            <div className="space-y-2">
              <Label htmlFor="skill-department">Department</Label>
              <Select
              ...
              >
                <SelectTrigger id="skill-department">
              ...
              <p className="text-xs text-muted-foreground">
                Who owns this skill. Use &ldquo;Sharing &amp; visibility&rdquo; below to also
