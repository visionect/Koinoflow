...
import { VersionTimeline } from "@/components/processes/VersionTimeline"
import { ScopeDialog } from "@/components/processes/ScopeDialog"
...
export function ProcessViewPage() {
  ...
  const canWrite = processQuery.data?.visibility === "workspace" ? isAdmin : isEditor
  const versionsQuery = useVersions(processSlug ?? "")
  const membersQuery = useWorkspaceMembers()
  const updateProcess = useUpdateProcess(processSlug ?? "")
  const publishProcess = usePublishProcess(processSlug ?? "")
  const reviewProcess = useReviewProcess(processSlug ?? "")
  const deleteProcess = useDeleteProcess()
  const unshareFromMyTeam = useUnshareProcessFromMyTeam(processSlug ?? "")
  const createVersion = useCreateVersion(processSlug ?? "")
  ...
  const department = departmentLookupQuery.data?.find(
    (item) =>
      item.slug === processQuery.data?.department_slug &&
      item.team_slug === processQuery.data?.team_slug,
  )
  const sharedDepartments = React.useMemo(() => {
    if (!departmentLookupQuery.data || !processQuery.data?.shared_with_ids?.length) return []
    const ids = new Set(processQuery.data.shared_with_ids)
    return departmentLookupQuery.data.filter((d) => ids.has(d.id))
  }, [departmentLookupQuery.data, processQuery.data?.shared_with_ids])
  ...
  const [frontmatter, setFrontmatter] = React.useState<ProcessFrontmatter>({
    name: "",
    description: "",
    tags: [],
  })
  ...
  const allProcessesQuery = useProcesses({ limit: 200 })
  const prerequisiteSuggestions = React.useMemo(
    () =>
      (allProcessesQuery.data?.items ?? [])
        .map((p) => p.slug)
        .filter((slug) => slug !== processSlug),
    [allProcessesQuery.data, processSlug],
  )
  ...
  const hasUnpublishedDraft =
    ...
    latestVersionNumber > (processQuery.data?.current_version?.version_number ?? 0)
  const displayVersion = hasUnpublishedDraft
    ? (latestVersionQuery.data ?? processQuery.data?.current_version ?? null)
    : (processQuery.data?.current_version ?? latestVersionQuery.data ?? null)
  const isViewingDraft =
    displayVersion !== null &&
    displayVersion.version_number !== processQuery.data?.current_version?.version_number

  const viewVersionNumber = displayVersion?.version_number ?? null
  const viewFilesQuery = useVersionFiles(processSlug ?? "", viewVersionNumber)
  ...
  React.useEffect(() => {
    if (processQuery.data) {
      ...
      setOwnerId(processQuery.data.owner?.id ?? "unassigned")
    }
  }, [processQuery.data])

  const initialVersion = latestVersionQuery.data ?? processQuery.data?.current_version ?? null
  ...
  React.useEffect(() => {
    ...
    if (initialVersion) {
      ...
      const scaffoldFrontmatter: ProcessFrontmatter = {
        name: processQuery.data.slug,
        description: processQuery.data.description,
        tags: [],
      }
      const scaffoldMetadata: KoinoflowMetadata = { ...EMPTY_KOINOFLOW_METADATA }
      const scaffoldMd = `# ${processQuery.data.title}\n\nDescribe this process, prerequisites, and critical steps here.`
  ...
  }, [initialVersion, initializedVersion, processQuery.data, hasNoVersions])
  ...
  const handleSave = React.useCallback(async () => {
    ...
    try {
      const nameAsTitle = frontmatter.name || processQuery.data.title
      if (
        nameAsTitle !== processQuery.data.title ||
        frontmatter.description !== processQuery.data.description
      ) {
      ...
      const message = error instanceof Error ? error.message : "Unable to save process"
  ...
  async function handleUpdateMetadata() {
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Unable to update process")
    }
  }
  ...
  async function handlePublish() {
    try {
      ...
      await publishProcess.mutateAsync()
      toast.success(
        `Published ${latestVersionNumber ? `v${latestVersionNumber}` : "latest version"}`,
      )
      setPublishOpen(false)
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Unable to publish process")
    }
  }
  ...
  async function handleReview() {
    try {
      await reviewProcess.mutateAsync()
  ...
  async function handleDelete() {
    ...
    try {
      await deleteProcess.mutateAsync(processQuery.data.slug)
      toast.success("Process deleted")
      navigate(buildWorkspacePath(workspace, "/processes"), { replace: true })
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Unable to delete process")
    }
  }
  ...
  async function handleUnshareFromMyTeam() {
    try {
      ...
      toast.success("Process removed from your team")
  ...
  async function handleExportSkill() {
    ...
    const skillName = processQuery.data.slug
    try {
      const response = await fetch(`/api/v1/processes/${skillName}/export`, {
        credentials: "include",
      })
  ...
  if (!processQuery.data) {
    return <EmptyState title="Loading process" description="Fetching process details..." />
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        title={processQuery.data.title}
        description={processQuery.data.description || "No description yet."}
        action={
          <div className="flex flex-wrap gap-2">
            {isEditing ? (
              <>
                <Button
                  disabled={
                    !hasUnsavedChanges || createVersion.isPending || updateProcess.isPending
                  }
                  onClick={() => void handleSave()}
                  title="Save new version (⌘/Ctrl+S)"
                >
                  <SaveIcon aria-hidden />
                  {createVersion.isPending || updateProcess.isPending
                    ? "Saving…"
                    : hasUnsavedChanges
              ...
              <>
                ...
                {canWrite && (needsLatestDraft || processQuery.data.status === "draft") ? (
                  <Button
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem
                  onClick={() =>
                    navigate(
                      buildWorkspacePath(
                        workspace,
                        `/processes/${processQuery.data?.slug}/history`,
                      ),
                    )
                  }
                >
                ...
                {isTeamManager && processQuery.data.is_shared_with_requester_team ? (
                  <DropdownMenuItem onClick={() => setUnshareOpen(true)}>
                    <UserMinusIcon className="mr-2 size-4" />
                    Remove from my team
                  </DropdownMenuItem>
                ) : null}
                {canWrite && processQuery.data.status === "published" ? (
      ...
      {isEditing ? (
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
          <div className="space-y-4">
            {processQuery.data.needs_audit ? (
              <Card className="border-destructive/30 bg-destructive/5">
                <CardContent className="flex items-center justify-between gap-4 pt-6">
                  <div className="flex items-center gap-3 text-sm text-destructive">
                    <AlertTriangleIcon className="size-5 shrink-0" />
                    <span>
                      This process is overdue for validation.
                      {processQuery.data.owner?.id !== user?.id
                        ? " Only the process owner can mark it validated."
                        : ""}
                    </span>
                  </div>
                  {user && processQuery.data.owner?.id === user.id ? (
                    <Button
                      size="sm"
                      variant="outline"
                      className="shrink-0 border-destructive/30 text-destructive hover:bg-destructive/10"
                      disabled={reviewProcess.isPending}
                      onClick={() => void handleReview()}
                    >
                      <CheckCircle2Icon className="mr-1 size-4" aria-hidden />
                      {reviewProcess.isPending ? "Validating…" : "Mark validated"}
                    </Button>
                  ) : null}
                </CardContent>
              </Card>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {(viewFilesQuery.data?.length ?? 0) > 0 && (
                        <div className="flex rounded-md border text-xs">
                          <button
                            className={`px-3 py-1.5 ${activeTab === "process" ? "bg-primary text-primary-foreground rounded-l-md" : "text-muted-foreground hover:text-foreground"}`}
              ...
              <CardContent className="space-y-4 text-sm">
                ...
                <div className="space-y-1">
                  ...
                  <p className="font-medium">
                    {processQuery.data.current_version
                      ? `Published v${processQuery.data.current_version.version_number}`
                      : latestVersionNumber
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    <p className="text-muted-foreground">
                      {processQuery.data.visibility === "team" ||
                      processQuery.data.visibility === "workspace"
                        ? "Deployment"
                  ...
                  {processQuery.data.visibility === "team" ? (
                    <Button asChild className="h-auto p-0 text-left" variant="link">
                      <Link
                        to={buildWorkspacePath(workspace, `/teams/${processQuery.data.team_slug}`)}
                      >
                        {processQuery.data.team_name} (team-wide)
                      </Link>
                    </Button>
                  ) : processQuery.data.visibility === "workspace" ? (
                    ...
                    department ? (
                      <Button asChild className="h-auto p-0 text-left" variant="link">
                        <Link to={buildWorkspacePath(workspace, `/depts/${department.id}`)}>
                          {processQuery.data.team_name} / {processQuery.data.department_name}
                        </Link>
                      </Button>
                    ) : (
                      <p className="font-medium">
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="process-title-edit">Title</Label>
              <Input
                id="process-title-edit"
                value={title}
                onChange={(event) => setTitle(event.target.value)}
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="process-description-edit">Description</Label>
              <Textarea
                id="process-description-edit"
                value={description}
          ...
          <DialogFooter>
            ...
            <Button
              disabled={!title.trim() || updateProcess.isPending}
      ...
      <Dialog open={publishOpen} onOpenChange={setPublishOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              disabled={publishProcess.isPending || updateVersionSummary.isPending}
            ...
            >
              ...
              {publishProcess.isPending || updateVersionSummary.isPending
                ? "Publishing..."
                : "Publish"}
            </Button>
          </DialogFooter>
      ...
      <DeleteConfirmDialog
        ...
        entityName={processQuery.data.title}
        description="This permanently deletes the process and all of its version history."
      ...
      <DeleteConfirmDialog
        ...
        description="Your team members will no longer see this process in their list. The process remains available to other teams it is shared with."
